...
def get_spectrogram(filename, samplerate=0):
    ...
    # numpy array to store spectrogram
    ...
    while True:
        ...
        specgram = vstack((specgram, pv(samples).norm)
                          )   # store new norm vector
        if read < a.hop_size:
            break

    # plotting
    fig = plt.imshow(log10(specgram.T + .001), origin='bottom',
    ...
    def get_rounded_ticks(top_pos, step, n_ticks):
        ...
        ticks_labels = [ticks_first_label *
                        n for n in range(n_ticks)] + [top_label]
        # get the corresponding positions
        ticks_positions = [ticks_labels[n] /
                           step for n in range(n_ticks)] + [top_pos]
        # convert to string
    ...
    y_ticks, y_labels = get_rounded_ticks(
        len(specgram[0]), (samplerate / 1000. / 2.) / len(specgram[0]), n_yticks)
...
if __name__ == '__main__':
    ...
    fig = get_spectrogram("./datasets/starter/track1.wav")
    # display graph
    plt.show()
